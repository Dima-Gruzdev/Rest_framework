...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        user1 = settings.AUTH_USER_MODEL.objects.get(
        ...
        Payments.objects.create(
            user=user1, paid_course=course, amount=15000.00, payment_method="transfer"
        )

        Payments.objects.create(
            user=user2, paid_lesson=lesson, amount=2500.50, payment_method="cash"
        )
